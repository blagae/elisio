...
class TestTextDecorator(unittest.TestCase):
    def test_dec_basic(self):
        verse = VerseFactory.create("Arma virumque cano, Troiae qui primus ab oris", False, False, classes=VerseType.HEXAMETER)
    ...
    def test_dec_bracket(self):
        verse = VerseFactory.create("Urbs antiqua fuit (Tyrii tenuere coloni)", False, False, classes=VerseType.HEXAMETER)
        s = TextDecorator(verse).decorate()
        self.assertEqual(s, "U̱rbs a̱nti̱qua̯ fu̯i̱t (Ty̯ri̯i̱ te̱nue̱re̯ co̯lo̱ni̱)")

    def test_dec_ancepsfinal(self):
        verse = VerseFactory.create("nos patriae finis et dulcia linquimus arva.", False, False, classes=VerseType.HEXAMETER)
        s = TextDecorator(verse).decorate()
