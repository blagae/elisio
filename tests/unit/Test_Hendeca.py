...
from elisio.parser.hendeca import (AlcaicHendeca, HendecaException,
                                   PhalaecianHendeca, SapphicHendeca,
                                   get_hendeca_subtype)
from elisio.syllable import Weight


def parse(lst):
    hen_class = get_hendeca_subtype(lst)
    hen_obj = hen_class('')
...
class TestHendecaCreator(unittest.TestCase):
    ...
    def test_hen_create_pha_easy(self):
        ...
        self.assertEqual(get_hendeca_subtype(sylls), PhalaecianHendeca)

    def test_hen_create_sap_easy(self):
        sylls = [Weight.ANCEPS] * 11
        sylls[5] = Weight.LIGHT
        self.assertEqual(get_hendeca_subtype(sylls), SapphicHendeca)

    def test_hen_create_alc_easy_1(self):
        sylls = [Weight.ANCEPS] * 11
        sylls[2] = Weight.LIGHT
        self.assertEqual(get_hendeca_subtype(sylls), AlcaicHendeca)
